...
**退出登录** 向同源 `/api/logout` 发 POST，清除团队会话与 dsh 实例 cookie，然后跳转到 `/`，此时未认证入口提供登录页。**生成配对码** 向同源 `/api/pairings` 发 POST（经代理转给账号服务、会话鉴权），并展示签发的配对码、认领命令与复制控件；该码在 TTL 内可复用，因此一个码可挂载多台设备。
