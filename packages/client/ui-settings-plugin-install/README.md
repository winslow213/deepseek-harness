...
`dsh-client-ui-settings-plugin-install` contributes the **Install plugin** tab to the Web Settings Plugins section. It offers four install forms — copying a local plugin directory, running `pnpm add` on an npm package, uploading a picked directory, or registering an installed npm plugin's startup row — submitted to `ctx.remote.pluginInstall.installPlugin()`; the form locks and shows progress, then reports the written profile directory and plugin id, or the Remote error. A second section uninstalls a plugin by its install-time id via `ctx.remote.pluginInstall.uninstallPlugin()`, covering all four forms. Both actions take effect only after the Web instance restarts. The tab registers only when `DSH_PLUGIN_INSTALL=true` is set.
...
The **Uninstall a plugin** section takes the plugin id used at install time — a patch-row id (`file-dir`, `upload-directory`, `npm-register`) or an npm-bundle package name — and submits it to `ctx.remote.pluginInstall.uninstallPlugin()`. A successful uninstall reports the profile directory and the removed plugin id, followed by the restart note; a refused uninstall (for example, an id with no installed plugin) reports the Remote error message and its error code.
