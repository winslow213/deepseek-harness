---
description: "面向 Web UI 的运算符门控插件安装 Remote：把外部插件安装到运行中 dsh profile 的 pluginInstall 服务及其 install Remote，含 file-dir 拷贝与 npm-bundle 两种形式。"
...
Web 实例可以通过 Remote 命名空间把外部插件安装进自己的 profile 目录：以 `file-dir` 形式调用 `pluginInstall/installPlugin` 会把源目录拷贝到 profile 的 `plugins/<id>` 下，并在用户 patch 层登记其 patch 行；`npm-bundle` 形式则在 profile 目录运行 `pnpm add`，并把 bundle 提升进 profile 的 `dsh.profile.bundles` 层列表。该服务受运算符门控：`PluginInstallGateway` 类在 `enabled: true` 之外拒绝挂载，且 web-app 组合在运算符设置 `DSH_PLUGIN_INSTALL=true` 之前禁用整行，因此默认部署从不加载该包。Client 包通过显式的 [`api-remotes`](../../api/remotes/README.zh.md) 组合消费这个 Remote，而不导入 Host 实现。
...
- **`npm-bundle`**——在 profile 目录运行 `pnpm add <spec>`（目录还没有 profile 清单时先初始化），然后调和 profile 的层栈，把新加入的 bundle 提升进 `dsh.profile.bundles`，使 bundle 层列表反映本次安装。
...
安装失败抛出带稳定码的 `RemoteError`：`plugin-install/unknown-profile`（无法定位 profile）、`plugin-install/invalid-spec`（id 不安全或源路径不可用）、`plugin-install/pnpm-missing`（PATH 上没有 pnpm）、`plugin-install/pnpm-failed`（`pnpm add` 非零退出）、`plugin-install/write-failed`（拷贝、清单、patch 或调和写入失败）。
...
网关把磁盘上的 profile 作为唯一安装真源：`file-dir` 安装直接写拷贝与 patch 行；`npm-bundle` 安装把包解析委托给 `pnpm`，让已调和的 profile `node_modules`——它已链接盒内与新增的 bundle——成为解析根。bundle 簿记由 `dsh-app-boot` 负责：`initProfile` 给缺失的 profile 清单播种，`readProfileManifest` 在安装前快照层状态，`reconcileProfileBundles` 把新加入的 bundle 提升进 profile 层列表。本服务不自做任何 npm 解析。
...
| 文件 | 职责 |
|---|---|
| [`src/index.ts`](src/index.ts) | `PluginInstallGateway`：`pluginInstall` Remote 服务、profile 自行定位、两种安装形式与 patch 行写入器 |
...
当安装约定不够用时阅读以下内容：先看 Remote 如何到达客户端，再看两种安装形式所依赖的 profile 与 bundle 机制。
