...
**The `grep`/`glob` escalation is open.** The bug fix note records the fence decision and the accepted regression risk; the search tool still spawns `rg` unconfined.
