...
- **默认浏览整个文件系统，可选加一个围闭根（root）。** 后端默认列出宿主主目录并接受任意完全限定路径（最初采用它的部署没有按账户的边界）。配置 `root` 会把 `list` 与 `createDirectory` 围闭到该子树内：列表的 `home` 锚点与面包屑都根植于此，其上的祖先永不作为跳转目标暴露，子树之外的目标被拒绝（`directory-unreadable` / `directory-create-failed`）。该边界是词法性的（防 `..` 的前缀检查，不做符号链接遍历），与团队 shell 的 region-router 读边界一致——不是内核保证。多账户团队 shell（2026-09-07）正是那个让此前搁置的 root 可配置性物有所值的部署。
