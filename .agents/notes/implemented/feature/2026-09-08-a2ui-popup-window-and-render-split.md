...
- **Open inside the click gesture.** `A2uiStorePanel` opens the named popup window directly inside its click handler, so no launcher `window.open` has run to capture a reference. The popup announces `a2ui/ready` with the `surfaceId` it reads from its own window name, and the launcher that a re-render projects adopts that popup by matching the announcement's `surfaceId` and taking `event.source` as its tracked window. This keeps the browser from blocking the popup while completing the handshake.
