...
**1. Data-source bindings.** Implemented — see [A2UI dynamic data-source provider seam](../../implemented/feature/2026-09-09-a2ui-data-source-provider.md). A field (initially `select`; later read-only text derived from a source) may declare `source` instead of static `options`. The shipped implementation resolves a `source` through a host-backed **data provider** (the `bash`-based provider runs one whitelisted read command), returning options through the Remote round-trip rather than the durable `a2ui/data` event this proposal described; that event is deferred because the options list is not model-visible.

**2. Live result channel.** Implemented — see [A2UI live-result stream](../../implemented/feature/2026-09-09-a2ui-live-result-stream.md). The shipped `a2ui/update` event carries `{ surfaceId, phase, seq, delta?, totalBytes? }`; a `command` action emits it from `ctx.a2uiRun`, and a `model` action's background job emits it through the explicit `a2ui_attach_output` binding plus an independent jobs reader — rather than the tool-executor reuse of the single `readOutput` cursor this proposal described, because that cursor belongs to `job_output`. The event is log-only, so the `ignorable: true` marker this proposal planned is not needed (a log-only event does not carry surface metadata and needs no version bump).

**3. Scripted local actions.** Implemented — see [A2UI scripted local actions](../../implemented/feature/2026-09-09-a2ui-scripted-local-actions.md). Custom logic beyond one expression becomes a declarative **step list** on a `local` action: `set` (assign one field), `append` (concatenate into a target), `refresh` (re-issue a data source), `stop` (terminate the correlated job). Each step is a restricted expression or a source name — the same evaluator grammar the browser already trusts, executed in order by the existing form/canvas host. Arbitrary JavaScript is explicitly out of scope: the page must never run model-authored code, only declared operations the renderer can log and reason about.
