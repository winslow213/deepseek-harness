...
const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  ...
  'packages/client/ui-a2ui': { kind: 'none', reason: 'Browser-side UI plugin layer; renders durable A2UI surface records as interactive Chat nodes without changing model context.' },
  'packages/client/ui-a2ui-render': { kind: 'none', reason: 'Zero-cordis A2UI renderer library; draws pages and posts collected values to the opener, which owns model serialization.' },
  'packages/client/ui-a2ui-store': { kind: 'none', reason: 'Browser-side UI plugin layer; the sidebar panel lists and removes saved A2UI tools without changing model context.' },
  ...
  'packages/client/ui-settings-plugin-install': { kind: 'none', reason: 'Browser-side install form; registers nothing model-facing.' },
  'packages/client/ui-team-account': { kind: 'none', reason: 'Browser-side sign-out row gated on the team-shell marker; registers nothing model-facing.' },
  ...
  'packages/host/plugin-install': { kind: 'none', reason: 'The operator-gated install Remote mutates the profile on disk; it registers nothing model-facing.' },
