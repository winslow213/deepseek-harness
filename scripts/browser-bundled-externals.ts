...
/**
 * Rollup's `options` hook overrides `input` after Vite resolves the config
 * file, instead of passing it through `vite.build()`'s own config merge:
 * that merge treats a same-shaped `rollupOptions.input` object as another
 * layer to deep-merge (keyed by page name) rather than a replacement, so a
 * filtered-out, non-HTML entry from the config file's own `input` survives
 * as an extra unresolvable entry.
 */
function inputOverride(pages: readonly string[]) {
  return {
    name: 'dsh-input-override',
    options(opts: { input?: unknown }): { input?: unknown } {
      opts.input = Object.fromEntries(pages.map(page => [basename(page, '.html'), page]))
      return opts
    },
  }
}
...
async function collectShell(
  ...
  for (const path of globSync('apps/*/vite.config.ts', { cwd: root }).sort()) {
    ...
    await vite.build({
      root: dir,
      logLevel: 'error',
      // A plain array `input` assigns Rollup's commonjs plugin numeric entry
      // ids, which its own resolveId hook rejects (only string ids reach
      // `.endsWith`); `inputOverride` supplies a keyed object instead,
      // through the `options` hook rather than this config (see its doc).
      plugins: [inputOverride(pages), recorder(seen, workspaceNames, true)],
