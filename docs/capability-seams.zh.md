...
flowchart LR
  ...
  svc_a2uiStore["ctx.a2uiStore<br/>A2UI page tool store"]
  ...
  pkg_tool_a2ui_store --> svc_a2uiStoreController
...
| `ctx.a2uiStore` | `core` | [`tool-a2ui-store`](../packages/web/tool-a2ui-store) | - | - | - | 已保存 A2UI 页面的本地持久化：在 harness A2UI 工具目录下进行名称排序的列表、规范化保存与移除。 |
